...
function aplicarConfig() {
  ...
  $("school").textContent = store.get("quiz_nome") || "Colégio Exemplo";
  const cor = store.get("quiz_cor") || "#1e4fd8";
...
function mostrar() {
  ...
  respondida = false;
  $("num").textContent = `Pergunta ${atual + 1} de ${perguntas.length}`;
...
function revelar(p, marcada, ganho) {
  ...
  historico.push({ correta: p.correta, pergunta: p.pergunta, marcada: marcada || "Tempo esgotado", ok: certa });
  if (certa) acertos++;
  pontos += ganho;
  const titulo = certa ? `✔ Correto! +${ganho} pontos` : marcada ? "✘ Não foi dessa vez" : "⏱ Tempo esgotado";
  const fb = $("fb");
...
function finalizar() {
  ...
  $("review").innerHTML = historico.map(h => {
    ...
    return `<div class="rev"><div>${esc(h.pergunta)}</div>${res}</div>`;
  }).join("");
}
